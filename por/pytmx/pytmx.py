...
class TiledMap(TiledElement):
    ...
    def load(self):
        ...
        for node in etree.findall('objectgroup'):
            group = TiledObjectGroup(self, node)
            self.objectgroups.append(group)
            self.objectnames[group.name] = group
...
class TiledObjectGroup(TiledElement, list):
    ...
    def find(self, name):
        """
        Return a list of all matching TiledObjects.
        """
        return filter(lambda obj: obj.name == name, self)
